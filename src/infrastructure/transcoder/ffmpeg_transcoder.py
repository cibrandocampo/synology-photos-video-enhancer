...
class FFmpegTranscoder(Transcoder):
    ...
    def _build_qsv_command(self) -> List[str]:
        ...
        cmd_parts.extend(["-init_hw_device", f"qsv=qsv:{device_path}"])
        cmd_parts.extend(["-filter_hw_device", "qsv"])
        
        # Hardware acceleration
        cmd_parts.extend(["-hwaccel", "qsv"])
        cmd_parts.extend(["-hwaccel_output_format", "qsv"])
        
        # Input file
        cmd_parts.extend(["-i", self.transcoding.original_video.path])
        
        # Video filter: vpp_qsv with framerate and scaling
        vf = f"vpp_qsv=framerate={int(config.video_framerate)}:h={config.video_height}:w=trunc(oh*dar/2)*2,setsar=1"
