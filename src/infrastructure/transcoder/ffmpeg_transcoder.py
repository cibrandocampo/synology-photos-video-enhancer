...
class FFmpegTranscoder(Transcoder):
    ...
    def __init__(
        ...
        Args:
            ...
            hw_transcoding: User preference for hardware acceleration. When False, software
                encoding is forced regardless of available hardware. When True, the detected
                backend is used (falls back to software if none is available).
        """
        ...
        # Reconcile user preference with system capability:
        # hw_transcoding=False always forces software; hw_transcoding=True uses whatever
        # the system has (from_hardware_acceleration returns NONE when nothing is detected).
        if hw_transcoding:
            self.hardware_backend: HardwareBackend = (
                HardwareBackend.from_hardware_acceleration(
                    hardware_info.video_acceleration
                )
            )
        else:
            self.hardware_backend = HardwareBackend.NONE
        ...
        self.video_encoder: str = transcoding.configuration.video_codec.encoder(
            ...
            if self.hardware_backend != HardwareBackend.NONE
            else None
        )
    ...
    def transcode(self) -> bool:
        ...
        try:
            ...
            self.logger.warning(
                f"FFmpeg transcoding failed for {self.transcoding.original_video.path}:"
            )
    ...
    def _build_qsv_command(self) -> List[str]:
        ...
        # Video filter: format conversion and hardware upload (matching working command format)
        ...
        cmd_parts.extend(
            ["-threads", str(self.transcoding.configuration.execution_threads)]
        )
